...
namespace EjemploServidor
{
    ...
    public class ServidorEventArgs : EventArgs
    {
        ...
        // Juancito: 
        public ServidorEventArgs(IPEndPoint[] ep2) { ArrayEndPoint = ep2; }
        public IPEndPoint[] ArrayEndPoint { get; }
    }
    ...
    // Juancito: Evento personalizado para mostrar clientes, igual a servereventargs
    public class ClientesConectadosEventArgs : ServidorEventArgs
    {       
        public ClientesConectadosEventArgs(IPEndPoint[] ep) : base(ep)
        { EndPointCliente = ep; }
        public IPEndPoint[] EndPointCliente { get; }
    }

    public class DatosRecibidosPrivadosEventArgs : ServidorEventArgs
    {
        public DatosRecibidosPrivadosEventArgs() : base(ep)
        {
    ...
    public class Servidor
    {
        ...
        // Juancito:
        public event EventHandler<ClientesConectadosEventArgs> ClientesConectados;
        ...
        private void LeerSocket(IPEndPoint endPoint)
        {
            ...
            while (cliente.Socket.Connected)
            {
                try
                {
                    ...
                    //int cantidadRecibidaPrivada = cliente.Socket.ReceiveFrom(buffer, ref new);
                    ...
                    else if(cantidadRecibidaPrivada > 0)
                    {
                        var datosRecibidosPrivados = Encoding.UTF8.GetString(buffer, 0, cantidadRecibidaPrivada);
                        DatosRecibidos?.Invoke(this, new DatosRecibidosEventArgs(endPoint, datosRecibidosPrivados));
                    }
                    else
                    {
        ...
        // Juancito: método para mostrar clientes conectados
        public void MostrarClientesConectados()
        {

            IPEndPoint[] ArrayIPEndPoint = new IPEndPoint[clientes.Count];
            int i = 0;
            // Recorro todos los clientes conectados
            foreach (var cliente in clientes.Values)
            {
                var endPoint = cliente.Socket.RemoteEndPoint as IPEndPoint; // Obtenemos el IPEndPoint del cliente
                ArrayIPEndPoint[i] = endPoint; i++; // Lo almacenamos en un array del tipo IPEndPoint para luego usarlo
            }
            ClientesConectados?.Invoke(this, new ClientesConectadosEventArgs(ArrayIPEndPoint));
        }
    }
}
